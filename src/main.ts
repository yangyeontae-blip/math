...
world.onInteract = id => { if (!state) return; if (id.startsWith('tree')) world.attack(); else if (id === 'guide') openGuide(); else if (id === 'weapon' || id === 'outfit') openShop(id); else if (id === 'ride') openInventory('ride'); else if (id === 'pet') openPetShop(); else if (id === 'beauty') openBeauty(); else if (id === 'arena') openArena(); else if (id === 'journey') openStageMap(); else if (id === 'home') switchStage(0); else if (id === 'next') openNextGate(); else { const huntId = Number(id.slice(7)); startBattle(stageMonsters(state.journey.stage)[huntId].type, false, id); } };
...
function startBattle(monster: number, arena: boolean, id: string) {
  if (!state) return; battle = { encounter: new Encounter(monster, arena, state.level, undefined, arena ? 0 : state.journey.stage), id, round: 1, score: 0, result: null }; renderBattle();
}
...
function openPetShop() {
  ...
  openModal(title('윤준의 펫 상점', '베리를 찾아주는 다섯 친구') + `<p class="shop-explainer">펫은 가까운 베리를 자동으로 먹어 줘요. <b>🍓 ${s.berries.toLocaleString()}베리</b></p><div class="inventory-list">${cards}</div><p class="error" id="pet-error" role="alert"></p>`, 'inventory-modal');
  document.querySelectorAll<HTMLButtonElement>('[data-buy-pet]').forEach(button => button.onclick = () => { try { const msg = buyPet(s, Number(button.dataset.buyPet)); audio.play('buy'); syncAvatar(); refresh(); persist(); openPetShop(); toast(msg); } catch (e) { $('#pet-error').textContent = (e as Error).message; } });
}
...
function openBeauty(kind: 'hairstyle' | 'face' = 'hairstyle', selectedId?: number) {
  ...
  preview = new AvatarPreview($('#shop-avatar')); preview.show(s.character, s.outfit, s.weapon, s.outfits[s.outfit], s.weapons[s.weapon], kind === 'hairstyle' ? id : s.hairstyle, kind === 'face' ? id : s.face);
