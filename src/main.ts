...
import { newSave, validateSave, CHARACTERS, MONSTERS, OUTFITS, WEAPONS, WEAPON_UPGRADES, OUTFIT_UPGRADES, Encounter, grantReward, rewardFor, buy, upgrade, collectBerry, finishHunt, canEnter, type Save } from './rules';
...
root.innerHTML = `<div id="world" aria-label="베리숲 3D 마을"></div><div id="hud" hidden>
  ...
  <div class="controls-help"><kbd>W A S D</kbd> 이동 <kbd>Space</kbd> 점프 <kbd>E</kbd> 대화</div>
  <button id="interact" class="interaction" hidden></button>
  <div id="touch-controls"><div id="joystick" role="group" aria-label="이동 조이스틱"><span id="stick"></span></div><div class="touch-actions"><button id="touch-talk">대화</button><button id="touch-jump">점프 ↟</button></div></div>
...
function refresh() {
  ...
  $('#level').textContent = `${s.level}`; $('#nickname').textContent = s.nickname; $('#berries').textContent = s.berries.toLocaleString();
  ...
  $('#weapon-effect').textContent = `대련 ×${(WEAPONS[s.weapon].multiplier + s.weapons[s.weapon] * .1).toFixed(1)} · 옷: ${OUTFITS[s.outfit].effect}`;
...
function syncAvatar() { if (state) world.setAvatar(state.character, state.outfit, state.weapon, state.outfits[state.outfit], state.weapons[state.weapon]); }
...
world.onNear = name => { $('#interact').hidden = !name; $('#interact').textContent = name ? `${name}  ·  대화하기 E` : ''; $('#touch-talk').textContent = name?.includes('슬라임') || name?.includes('요정') || name?.includes('토끼') || name?.includes('정령') ? '대련' : '대화'; };
world.onInteract = id => { if (!state) return; if (id === 'guide') openGuide(); else if (id === 'weapon' || id === 'outfit') openShop(id); else if (id === 'arena') openArena(); else if (id === 'journey') openStageMap(); else if (id === 'home') switchStage(0); else if (id === 'next') openNextGate(); else { const huntId = Number(id.slice(7)); startBattle(stageMonsters(state.journey.stage)[huntId].type, false, id); } };
$('#interact').onclick = () => world.interact(); $('#touch-talk').onclick = () => world.interact(); $('#touch-jump').onpointerdown = e => { e.preventDefault(); world.jump(); };
...
function renderBattle() {
  ...
  openModal(title(e.arena ? `별솔의 대련장 · ${b.round} / 5` : '숲속 친구와 나눗셈', m.name) + `<div class="battle-top"><span>${m.icon} ${e.arena ? `이번 도전 ${b.score}점` : '천천히 생각해도 괜찮아'}</span><span>🍓 ${reward.berries} · 경험치 ${reward.xp}</span></div><div class="monster-portrait" id="monster-portrait">${m.icon}<span>문제를 풀면 내가 응원해 줄게!</span></div><div class="question" aria-label="${q.dividend} 나누기 ${q.divisor}"><b>${q.dividend}</b><span>÷</span><b>${q.divisor}</b><span>=</span><input id="answer" aria-label="나눗셈의 답" inputmode="numeric" autocomplete="off" maxlength="2" placeholder="?" readonly></div><p class="answer-message" id="answer-message" role="status">몇씩 나누어 줄 수 있을까요?</p><div id="hint" hidden></div><div class="number-pad" aria-label="숫자판">${[1, 2, 3, 4, 5, 6, 7, 8, 9, '지우기', 0, '확인'].map(n => `<button data-number="${n}" class="${n === '확인' ? 'primary' : ''}">${n}</button>`).join('')}</div><div class="battle-footer"><button id="show-hint" class="text-button">💡 힌트 보기</button><button class="text-button" data-close>잠깐 쉬기</button></div><div id="battle-result" hidden></div>`, 'battle-modal');
...
function showHint() {
  ...
  $('#hint').hidden = false;
  $('#hint').innerHTML = `<p>${q.dividend}개를 ${q.divisor}묶음에 똑같이 나눠요.</p><div class="groups">${Array.from({ length: q.divisor }, () => `<div class="group"><span>${Array.from({ length: q.answer }, () => '<i></i>').join('')}</span><small>한 묶음</small></div>`).join('')}</div><b>${q.divisor} × □ = ${q.dividend}</b><p>한 묶음 안의 점을 세어 보세요.</p>`;
}
...
function openShop(kind: 'weapon' | 'outfit', selectedId?: number) {
  ...
  openModal(title(kind === 'weapon' ? '강지후의 무기 상점' : '오지후의 의상 상점', kind === 'weapon' ? '모험의 단짝을 골라요' : '귀여운 힘을 골라요') + `<p class="shop-explainer">${kind === 'weapon' ? '무기에 따라 받는 베리와 대련 점수가 달라져요.' : '의상마다 베리, 경험치, 스테이지 완료 보너스가 달라져요.'} <b>🍓 ${s.berries}베리</b></p><div class="shop-layout"><div class="item-grid">${items.map((it, i) => `<button class="item-card ${i === id ? 'selected' : ''}" data-item="${i}" aria-pressed="${i === id}"><span class="item-swatch" style="--item-color:#${it.color.toString(16).padStart(6, '0')}">${kind === 'weapon' ? WEAPONS[i].icon : '✿'}</span><strong>${it.name}</strong><small>${Object.hasOwn(owned, i) ? s[kind] === i ? '✓ 입고 있어요'.replace('입고', kind === 'weapon' ? '들고' : '입고') : `보유 · +${owned[i]}` : `🍓 ${it.price}베리`}</small></button>`).join('')}</div><div class="item-detail"><div id="shop-avatar"></div><span class="preview-tag">미리 보기</span><h3>${item.name}${level !== undefined ? ` +${level}` : ''}</h3><p>${kind === 'weapon' ? `몬스터 보상 베리 +${WEAPONS[id].bonus}<br>대련 점수 ×${(WEAPONS[id].multiplier + (level ?? 0) * .1).toFixed(1)}` : `${OUTFITS[id].desc}<br><b>귀여운 효과: ${OUTFITS[id].effect}</b>`}</p><button class="primary wide" id="buy-item" ${s[kind] === id ? 'disabled' : ''}>${s[kind] === id ? '지금 장착 중' : level !== undefined ? '장착하기' : `${item.price}베리로 구매·장착`}</button>${level !== undefined ? `<button class="secondary wide" id="upgrade-item" ${level >= (kind === 'weapon' ? 3 : 2) ? 'disabled' : ''}>${level >= (kind === 'weapon' ? 3 : 2) ? '최고 단계예요 ✦' : `강화하기 · ${(kind === 'weapon' ? WEAPON_UPGRADES : OUTFIT_UPGRADES)[level]}베리`}</button><small class="upgrade-note">${kind === 'weapon' ? '강화마다 대련 점수 배율 +0.1' : '강화하면 자수와 반짝임이 더해져요'}</small>` : ''}<p class="error" id="shop-error" role="alert"></p></div></div>`, 'shop-modal');
