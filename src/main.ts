...
import { newSave, validateSave, CHARACTERS, MONSTERS, OUTFITS, WEAPONS, WEAPON_UPGRADES, OUTFIT_UPGRADES, Encounter, grantReward, rewardFor, buy, upgrade, collectBerry, finishHunt, fellTree, treeDamage, canEnter, type Save } from './rules';
...
function refresh() {
  ...
  $('#weapon-effect').textContent = `대련 ×${(WEAPONS[s.weapon].multiplier + s.weapons[s.weapon] * .1).toFixed(1)} · 나무 힘 ${treeDamage(s)} · 옷: ${OUTFITS[s.outfit].effect}`;
...
world.onInteract = id => { if (!state) return; if (id.startsWith('tree')) world.attack(); else if (id === 'guide') openGuide(); else if (id === 'weapon' || id === 'outfit') openShop(id); else if (id === 'arena') openArena(); else if (id === 'journey') openStageMap(); else if (id === 'home') switchStage(0); else if (id === 'next') openNextGate(); else { const huntId = Number(id.slice(7)); startBattle(stageMonsters(state.journey.stage)[huntId].type, false, id); } };
