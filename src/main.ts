...
function startBattle(monster: number, arena: boolean, id: string) {
  if (!state) return; const cleared = state.journey.maps.slice(1).filter(map => map.cleared).length, difficultyStage = arena ? Math.min(10, cleared + 1) : state.journey.stage; battle = { encounter: new Encounter(monster, arena, state.level, undefined, difficultyStage), id, round: 1, score: 0, result: null }; renderBattle();
}
...
function nextBattle() {
  ...
  const previous = battle.encounter.question, difficultyStage = battle.encounter.stage; battle.round++; battle.encounter = new Encounter(Math.floor(Math.random() * 4), true, state.level, previous, difficultyStage); battle.result = null; renderBattle();
}
